<?php
namespace PawFriendsCore\CPT\Shortcodes\ImageWithText;
...
class ImageWithText implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Image With Text', 'pawfriends-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'description' => esc_html__( 'Enter image size. Example: thumbnail, medium, large, full or other sizes defined by current theme. Alternatively enter image size in pixels: 200x100 (Width x Height). Leave empty to use "thumbnail" size', 'pawfriends-core' )
						),
                        array(
                            'type'       => 'dropdown',
                            'param_name' => 'rounded_corners',
                            'heading'    => esc_html__( 'Rounded Corners', 'pawfriends-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Custom Link', 'pawfriends-core' ),
	...
	public function render( $atts, $content = null ) {
        ...
        $params['title_predefined']    = empty( $params['title_tag'] ) ? 'predefined' : '';
		$params['title_tag']           = ! empty( $params['title_tag'] ) ? $params['title_tag'] : 'h4';
        $params['title_highlight']     = ! empty( $params['title_highlight'] ) ? $params['title_highlight'] : $args['title_highlight'];
		$params['title_styles']        = $this->getTitleStyles( $params );
        $params['title_holder_styles'] = $this->getTitleHolderStyles( $params );
		$params['text_styles']         = $this->getTextStyles( $params );
		
		$html = pawfriends_core_get_shortcode_module_template_part( 'templates/image-with-text', 'image-with-text', '', $params );
	...
	private function getHolderClasses( $params ) {
        ...
        $holderClasses[] = $params['rounded_corners'] === 'yes' ? 'rounded' : '';
		$holderClasses[] = ! empty( $params['image_behavior'] ) ? 'mkdf-image-behavior-' . $params['image_behavior'] : '';
        $holderClasses[] = $params['title_highlight'] === 'yes' ? 'highlight' : '';
