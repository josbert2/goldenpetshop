<?php
namespace PawFriendsCore\CPT\Shortcodes\Counter;
...
class Counter implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Counter', 'pawfriends-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array(
								esc_html__( 'Zero Counter', 'pawfriends-core' )   => 'mkdf-zero-counter',
								esc_html__( 'Random Counter', 'pawfriends-core' ) => 'mkdf-random-counter'
							),
							'save_always' => true,
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'digit',
							'heading'    => esc_html__( 'Digit', 'pawfriends-core' )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Digit Color', 'pawfriends-core' ),
							'dependency' => array( 'element' => 'digit', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
	...
	public function render( $atts, $content = null ) {
		...
		wp_enqueue_script( 'counter', PAWFRIENDS_CORE_SHORTCODES_URL_PATH . '/counter/assets/js/plugins/counter.js', array( 'jquery' ), false, true );
		wp_enqueue_script( 'absoluteCounter', PAWFRIENDS_CORE_SHORTCODES_URL_PATH . '/counter/assets/js/plugins/absoluteCounter.min.js', array( 'jquery' ), false, true );
		
		$html = pawfriends_core_get_shortcode_module_template_part( 'templates/counter', 'counter', '', $params );
	...
	private function getCounterStyles( $params ) {
		...
		if ( ! empty( $params['digit_font_size'] ) ) {
			$styles[] = 'font-size: ' . pawfriends_mikado_filter_px( $params['digit_font_size'] ) . 'px';
		}
