...
class PawFriendsCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( PAWFRIENDS_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$pawfriends_import = new WP_Import();
		set_time_limit( 0 );
		
		$pawfriends_import->fetch_attachments = $this->attachments;
		$returned_value                  = $pawfriends_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'pawfriends-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'pawfriends-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $pawfriends_widget_id => $pawfriends_widget_data ) {
			update_option( 'widget_' . $pawfriends_widget_id, $pawfriends_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'pawfriends-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$pawfriends_sidebars = get_option( "sidebars_widgets" );
		unset( $pawfriends_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$pawfriends_sidebars = array_merge( (array) $pawfriends_sidebars, (array) $data['sidebars'] );
			unset( $pawfriends_sidebars['wp_inactive_widgets'] );
			$pawfriends_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $pawfriends_sidebars );
			$pawfriends_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $pawfriends_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'pawfriends-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'mkdf_options_pawfriends', $options );
		$this->message = esc_html__( 'Options imported successfully', 'pawfriends-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$pawfriends_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $pawfriends_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $pawfriends_page_option => $pawfriends_page_id ) {
			update_option( $pawfriends_page_option, $pawfriends_page_id );
		}
	}
	...
	function mkdf_admin_import() {
		if ( pawfriends_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
                MIKADO_OPTIONS_SLUG,
				esc_html__( 'PawFriends Options - PawFriends Import', 'pawfriends-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'pawfriends-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                MIKADO_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $pawfriends_mikado_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'pawfriends_mikado_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'pawfriends_mikado_enqueue_admin_styles' );
		}
	}
	...
	function mkdf_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'mkdf_options_pawfriends' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );
			
			update_option( 'mkdf_options_pawfriends', $new_global_values );
		}
	}
