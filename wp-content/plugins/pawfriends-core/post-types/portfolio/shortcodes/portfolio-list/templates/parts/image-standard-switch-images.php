...
<div class="mkdf-pli-image">
    ...
    <?php if (!empty($switch_featured_image)) {
        if ($thumb_size === 'full') { ?>
            <img itemprop="image" class="mkdf-pl-hover-image" src="<?php echo esc_url($switch_featured_image); ?>" alt="<?php esc_attr_e('Portfolio Hover Featured Image', 'pawfriends-core'); ?>" />
        <?php } else {
            $thumb_image_size = pawfriends_mikado_get_image_size($thumb_size);
            echo pawfriends_mikado_generate_thumbnail(null, $switch_featured_image, $thumb_image_size['width'], $thumb_image_size['height']);
        }
    } ?>
</div>