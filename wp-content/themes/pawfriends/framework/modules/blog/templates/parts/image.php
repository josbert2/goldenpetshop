...
$blog_list_image_id  = ! empty( $image_meta ) && pawfriends_mikado_blog_item_has_link() ? pawfriends_mikado_get_attachment_id_from_url( $image_meta ) : '';
?>
	...
	<div class="mkdf-post-image">
		...
		<?php } ?>
			<?php if ( $image_size !== 'custom' ) {
				...
				if ( ! empty( $blog_list_image_id ) ) {
					echo pawfriends_mikado_generate_thumbnail( $blog_list_image_id, null, intval( $custom_image_width ), intval( $custom_image_height ) );
				} else {
		...
		<?php } ?>
		<?php do_action('pawfriends_mikado_action_blog_inside_image_tag')?>
	</div>
<?php } ?>