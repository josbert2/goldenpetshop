...
class MikadoSkin extends PawFriendsMikadoClassSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to pawfriends_mikado_admin_scripts_init and pawfriends_mikado_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'mikado';
		
		// Admin register scripts and styles
		add_action( 'pawfriends_mikado_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'pawfriends_mikado_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'pawfriends_mikado_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'pawfriends_mikado_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'pawfriends_mikado_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'pawfriends_mikado_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'pawfriends_mikado_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			pawfriends_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            pawfriends_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= pawfriends_mikado_get_admin_tab();
        $active_page 	= $pawfriends_mikado_global_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see PawFriendsMikadoClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param PawFriendsMikadoClassAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(pawfriends_mikado_get_theme_info_item('Name'), pawfriends_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
