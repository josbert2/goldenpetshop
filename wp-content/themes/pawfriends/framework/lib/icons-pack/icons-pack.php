...
foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( pawfriends_mikado_is_customizer_item_enabled( $module_load, 'pawfriends_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'pawfriends_mikado_get_icon_pack_collections' ) ) {
	function pawfriends_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'pawfriends_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'pawfriends_mikado_get_icon_pack_options' ) ) {
	function pawfriends_mikado_get_icon_pack_options() {
		$options = apply_filters( 'pawfriends_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'pawfriends_mikado_get_icon_pack_social_options' ) ) {
	function pawfriends_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'pawfriends_mikado_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: PawFriendsMikadoClassIconCollections
 */
class PawFriendsMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = pawfriends_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = pawfriends_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = pawfriends_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = pawfriends_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'pawfriends' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'pawfriends' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'pawfriends' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'pawfriends' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'pawfriends' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'pawfriends' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'pawfriends' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'pawfriends' )       => 'fa-lg',
			esc_html__( 'Small', 'pawfriends' )      => 'fa-2x',
			esc_html__( 'Medium', 'pawfriends' )     => 'fa-3x',
			esc_html__( 'Large', 'pawfriends' )      => 'fa-4x',
			esc_html__( 'Very Large', 'pawfriends' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iPawFriendsMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'pawfriends' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'pawfriends' ) ] = '';
